<?php
/**
 * Mini Laravel 博客 — 自包含 PHP 博客应用
 * 
 * 模仿 Laravel MVC 架构，无需 Composer 依赖
 * 特性: SQLite 数据库 | 微型路由器 | CRUD | 响应式 UI
 */
...
function renderHome(array $posts, array $cats, string $activeCat): string {
    $postCards = '';
    foreach ($posts as $p) {
        $date = date('Y年m月d日', strtotime($p['created_at']));
        $postCards .= '
        <article class="card">
            ...
            <div class="card-meta">
                <span>' . $date . '</span>
                <a href="/post/' . $p['id'] . '" class="btn-sm">阅读更多 →</a>
            </div>
        </article>';
    }

    $catLinks = '<a href="/" class="tag ' . (!$activeCat ? 'active' : '') . '">全部</a>';
    foreach ($cats as $c) {
    ...
    <section class="hero">
        <h1>📝 Laravel 博客</h1>
        <p>用纯 PHP 构建的微型 MVC 博客 — 运行在你的 Android 设备上</p>
    ...
    <div class="toolbar">
        ...
        <a href="/create" class="btn">➕ 写文章</a>
    </div>
    <div class="grid">' . ($postCards ?: '<p class="empty">还没有文章，<a href="/create">写第一篇</a>吧！</p>') . '</div>';
}

function renderShow(array $post): string {
    $date = date('Y年m月d日 H:i', strtotime($post['created_at']));
    ...
    <div class="post-actions">
        ...
        <a href="/delete/' . $post['id'] . '" class="btn btn-danger" onclick="return confirm(\'\u786e定删除这篇文章吗？\')">🗑 删除</a>
    </div>';
}
...
function renderCreate(): string {
    ...
    <div class="form-card">
        ...
        <form method="POST" action="/store">
            ...
            <label>摘要</label>
            <input type="text" name="excerpt" placeholder="可选，留空则自动生成">
            <label>内容 (支持 Markdown)</label>
            <textarea name="content" rows="12" required placeholder="开始写作..."></textarea>
...
function renderNotFound(): string {
    return '<div class="empty-state"><h1>404</h1><p>页面未找到</p><a href="/" class="btn">返回首页</a></div>';
}
...
.post-content pre{background:rgba(0,0,0,0.3);color:#4ade80;padding:14px;border-radius:8px;overflow-x:auto;font-size:.82em;margin:10px 0;border:1px solid rgba(34,197,94,0.1)}
...
.post-content ul{padding-left:20px;margin:8px 0}
...
<footer>Mini Laravel 博客 · PHP ' . phpversion() . ' · 运行在 Android 设备上</footer>
</body>
</html>';
}
