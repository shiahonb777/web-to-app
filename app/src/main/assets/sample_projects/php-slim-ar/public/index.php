<?php
/**
 * Mini Slim — لوحة تحكم REST API مستقلة
 * 
 * بأسلوب Slim 4، بدون Composer
 * REST API | اختبار تفاعلي | SQLite | JSON
 */
...
try {
    ...
    if ($pdo->query('SELECT COUNT(*) FROM tasks')->fetchColumn() == 0) {
        $seed = [
            ['تصميم واجهات REST API', 1, 'high'],
            ['تنفيذ عمليات CRUD', 1, 'high'],
            ['إضافة التحقق من المدخلات', 0, 'medium'],
            ['كتابة وثائق API', 0, 'medium'],
            ['تحسين الأداء', 0, 'low'],
        ];
...
if (str_starts_with($uri, '/api/')) {
    ...
    if ($method === 'POST' && $uri === '/api/tasks') {
        ...
        if (!$title) jsonResponse(422, ['error' => 'العنوان مطلوب']);
        if ($pdo) {
        ...
        jsonResponse(500, ['error' => 'قاعدة البيانات غير متاحة']);
    }
    ...
    if ($method === 'PATCH' && preg_match('#^/api/tasks/(\d+)/toggle$#', $uri, $m)) {
        ...
        jsonResponse(404, ['error' => 'المهمة غير موجودة']);
    }
    ...
    jsonResponse(404, ['error' => 'نقطة النهاية غير موجودة: ' . $method . ' ' . $uri]);
}
...
body{font-family:-apple-system,BlinkMacSystemFont,Tahoma,sans-serif;background:var(--bg);color:var(--text);line-height:1.6;min-height:100vh;direction:rtl}
...
.endpoint{display:flex;align-items:center;gap:10px;padding:8px;border-bottom:1px solid rgba(255,255,255,0.03);cursor:pointer;border-radius:8px}
...
.ep-path{font-family:monospace;font-size:.85em;flex:1;color:var(--text);direction:ltr;text-align:left}
...
.response{margin-top:10px;background:rgba(0,0,0,0.3);color:#4ade80;border-radius:8px;padding:14px;font-family:monospace;font-size:.78em;white-space:pre-wrap;max-height:250px;overflow:auto;display:none;direction:ltr;text-align:left;border:1px solid rgba(34,197,94,0.1)}
...
<div class="hero">
    <h1>🚀 Slim REST API</h1>
    <p>خادم RESTful API مستقل — يعمل على جهاز Android</p>
    <div class="stats">
        <div class="stat"><div class="stat-num"><?=$taskCount?></div><div class="stat-label">إجمالي المهام</div></div>
        <div class="stat"><div class="stat-num"><?=$doneCount?></div><div class="stat-label">مكتملة</div></div>
        <div class="stat"><div class="stat-num">6</div><div class="stat-label">نقاط API</div></div>
...
<div class="container">
    <div class="section">
        ...
        <div class="endpoint" onclick="testApi('GET','/api/tasks')">
            ...
            <span class="ep-desc">جلب قائمة المهام</span>
        </div>
        <div class="endpoint" onclick="testApi('POST','/api/tasks','{&quot;title&quot;:&quot;مهمة جديدة&quot;,&quot;priority&quot;:&quot;high&quot;}')">
            <span class="method method-post">POST</span>
            <span class="ep-path">/api/tasks</span>
            <span class="ep-desc">إنشاء مهمة جديدة</span>
        </div>
        ...
        <div class="tester" id="tester">
            <div class="tester-row">
                ...
                <button class="btn btn-sm" onclick="sendRequest()">إرسال</button>
            </div>
            <input id="t-body" placeholder='Body (JSON): {"title":"مهمة جديدة"}' style="width:100%;padding:8px 12px;border:1px solid #ddd;border-radius:6px;font-size:.9em;margin-bottom:4px">
    ...
    <div class="section">
        <h2>✅ إدارة المهام</h2>
        <div class="add-form">
            <input id="new-title" placeholder="أدخل مهمة جديدة...">
            <select id="new-priority"><option value="high">عالي</option><option value="medium" selected>متوسط</option><option value="low">منخفض</option></select>
            <button class="btn btn-sm" onclick="addTask()">إضافة</button>
        </div>
        <div id="task-list">جار التحميل...</div>
    </div>
</div>
<footer>Mini Slim API · PHP <?=phpversion()?> · يعمل على Android</footer>
...
async function sendRequest() {
    ...
    } catch(e) { resp.innerHTML = 'خطأ: ' + e.message; }
}
function loadTasks() {
    fetch('/api/tasks').then(r => r.json()).then(data => {
        const list = $('task-list');
        if (!data.tasks.length) { list.innerHTML = '<p style="color:#999;text-align:center;padding:20px">لا توجد مهام، أضف واحدة!</p>'; return; }
...
function addTask() {
    ...
    if (!title) { inp.style.borderColor='#dc3545'; inp.placeholder='الرجاء إدخال عنوان المهمة!'; setTimeout(()=>{inp.style.borderColor='#ddd';inp.placeholder='أدخل مهمة جديدة...';},2000); return; }
    ...
    fetch('/api/tasks', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: body})
        ...
        .catch(e => { console.error('addTask:', e); inp.placeholder='فشل:'+e.message; });
}
