<?php
/**
 * Mini Slim — Self-contained REST API Dashboard
 * 
 * Slim 4 style, no Composer required
 * Features: RESTful API | Interactive Tester | SQLite | JSON
 */
...
try {
    ...
    if ($pdo->query('SELECT COUNT(*) FROM tasks')->fetchColumn() == 0) {
        $seed = [
            ['Design REST API endpoints', 1, 'high'],
            ['Implement database CRUD', 1, 'high'],
            ['Add input validation', 0, 'medium'],
            ['Write API documentation', 0, 'medium'],
            ['Performance optimization', 0, 'low'],
        ];
...
if (str_starts_with($uri, '/api/')) {
    ...
    if ($method === 'POST' && $uri === '/api/tasks') {
        ...
        if (!$title) jsonResponse(422, ['error' => 'Title is required']);
        if ($pdo) {
    ...
    jsonResponse(404, ['error' => 'Endpoint not found: ' . $method . ' ' . $uri]);
}
...
<title>Slim API Dashboard</title>
<style>
...
<div class="hero">
    <h1>🚀 Slim REST API</h1>
    <p>Self-contained RESTful API server — running on your Android device</p>
    <div class="stats">
        <div class="stat"><div class="stat-num"><?=$taskCount?></div><div class="stat-label">Total Tasks</div></div>
        <div class="stat"><div class="stat-num"><?=$doneCount?></div><div class="stat-label">Completed</div></div>
        <div class="stat"><div class="stat-num">6</div><div class="stat-label">API Endpoints</div></div>
...
<div class="container">
    <div class="section">
        ...
        <div class="endpoint" onclick="testApi('GET','/api/tasks')">
            ...
            <span class="ep-desc">Get task list</span>
        </div>
        <div class="endpoint" onclick="testApi('POST','/api/tasks','{&quot;title&quot;:&quot;New task&quot;,&quot;priority&quot;:&quot;high&quot;}')">
            <span class="method method-post">POST</span>
            <span class="ep-path">/api/tasks</span>
            <span class="ep-desc">Create new task</span>
        </div>
        ...
        <div class="tester" id="tester">
            <div class="tester-row">
                ...
                <button class="btn btn-sm" onclick="sendRequest()">Send</button>
            </div>
            <input id="t-body" placeholder='Body (JSON): {"title":"New task"}' style="width:100%;padding:8px 12px;border:1px solid #ddd;border-radius:6px;font-size:.9em;margin-bottom:4px">
    ...
    <div class="section">
        <h2>✅ Task Manager</h2>
        <div class="add-form">
            <input id="new-title" placeholder="Enter new task...">
...
<footer>Mini Slim API · PHP <?=phpversion()?> · Running on Android</footer>
...
function loadTasks() {
    fetch('/api/tasks').then(r => r.json()).then(data => {
        ...
        if (!data.tasks.length) { list.innerHTML = '<p style="color:#999;text-align:center;padding:20px">No tasks yet, add one!</p>'; return; }
...
function addTask() {
    ...
    if (!title) { inp.style.borderColor='#dc3545'; inp.placeholder='Please enter a task title!'; setTimeout(()=>{inp.style.borderColor='#ddd';inp.placeholder='Enter new task...';},2000); return; }
    ...
    fetch('/api/tasks', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: body})
        ...
        .catch(e => { console.error('addTask:', e); inp.placeholder='Failed:'+e.message; });
}
