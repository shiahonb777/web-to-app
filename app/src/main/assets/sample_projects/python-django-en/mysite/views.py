"""
Django Admin Dashboard — Beautiful Admin Panel
Showcases Django's MTV architecture and template rendering
"""
import os
import random
...
def dashboard(request):
    ...
    logs = [
        {'action': 'Created new post', 'user': 'Admin', 'time': f'{random.randint(2,30)} min ago', 'icon': '📝', 'color': '#3b82f6'},
        {'action': 'Updated user permissions', 'user': 'System', 'time': f'{random.randint(1,5)} hr ago', 'icon': '🔐', 'color': '#7c3aed'},
        {'action': 'Published 3 comments', 'user': 'Alex', 'time': f'{random.randint(1,3)} hr ago', 'icon': '💬', 'color': '#10b981'},
        {'action': 'Uploaded 5 images', 'user': 'Emily', 'time': f'{random.randint(2,6)} hr ago', 'icon': '🖼️', 'color': '#06b6d4'},
        {'action': 'Database backup', 'user': 'System', 'time': 'Today 08:00', 'icon': '💾', 'color': '#f59e0b'},
    ]
...
:root{{--bg:#0c0c1d;--card:rgba(255,255,255,0.035);--border:rgba(255,255,255,0.07);--text:#f0f0ff;--text2:rgba(240,240,255,0.55);--text3:rgba(240,240,255,0.3);--green:#0ea572;--django2:#44b78b;--radius:14px}}
...
<body>
<div class="c">
<div class="hdr"><div><h1>🦄 Django Admin</h1><p>Admin Dashboard</p></div><div class="badge"><span class="dot"></span>Running</div></div>
<div class="row4">
    <div class="s"><div class="s-h"><span class="s-label">Users</span><span class="s-icon">👥</span></div><div class="s-val">{stats['users']}</div><span class="s-tag">↑ 12.3%</span></div>
    <div class="s"><div class="s-h"><span class="s-label">Posts</span><span class="s-icon">📝</span></div><div class="s-val">{stats['posts']}</div><span class="s-tag">↑ 8.7%</span></div>
    <div class="s"><div class="s-h"><span class="s-label">Comments</span><span class="s-icon">💬</span></div><div class="s-val">{stats['comments']}</div><span class="s-tag">↑ 15.1%</span></div>
    <div class="s"><div class="s-h"><span class="s-label">Views</span><span class="s-icon">👁️</span></div><div class="s-val">{stats['pageviews']}</div><span class="s-tag">↑ 22.4%</span></div>
</div>
<div class="card">
    <div class="card-h"><span class="card-t">Weekly Activity</span><span class="card-b">📊 Trend</span></div>
...
<div class="card">
    <div class="card-h"><span class="card-t">Data Models</span><span class="card-b">📦 {len(models)}</span></div>
    {''.join(f'<div class="model"><div class="model-icon" style="background:{m["color"]}18">{m["icon"]}</div><div class="model-info"><div class="model-name">{m["name"]}</div><div class="model-count">{m["count"]} records</div></div></div>' for m in models)}
</div>
<div class="card">
    <div class="card-h"><span class="card-t">Activity Log</span><span class="card-b">🔔 Recent</span></div>
...
def api_stats(request):
    return JsonResponse({'status': 'ok', 'data': {'users': random.randint(8000,15000), 'posts': random.randint(2000,5000)}, 'framework': 'Django'})
