...
def generate_weekly_data():
    ...
    return [{'day': d, 'users': random.randint(180, 420)} for d in days]

def generate_activities():
    actions = [
        ('اشترى الخطة المميزة', '💎', '#7c3aed'),
        ('أكمل أول طلب', '🛒', '#3b82f6'),
        ('ترقية إلى المؤسسة', '🚀', '#10b981'),
        ('جدد الاشتراك السنوي', '🔄', '#f59e0b'),
        ('دعا 3 أعضاء فريق', '👥', '#ec4899'),
        ('صدّر تقرير البيانات', '📊', '#06b6d4'),
    ]
    names = ['أحمد', 'فاطمة', 'محمد', 'سارة', 'عمر', 'نور']
    result = []
    for i in range(6):
        action, icon, color = actions[i]
        m = random.randint(5, 720)
        time_str = f'{m} دقيقة' if m < 60 else f'{m//60} ساعة'
        result.append({'name': names[i], 'action': action, 'icon': icon, 'color': color, 'time': time_str})
    return result
...
body {
    ...
    min-height: 100vh; -webkit-font-smoothing: antialiased; direction: rtl;
}
body::before {
    content: ''; position: fixed; top: -50%; left: -50%; width: 200%; height: 200%;
    background: radial-gradient(ellipse at 70% 20%, rgba(124,58,237,0.08) 0%, transparent 50%),
                radial-gradient(ellipse at 30% 60%, rgba(59,130,246,0.06) 0%, transparent 50%);
...
.status-dot { width: 10px; height: 10px; border-radius: 50%; background: var(--accent-green); box-shadow: 0 0 12px var(--accent-green); animation: pulse 2s infinite; }
@keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.6} }
.avatar { width: 38px; height: 38px; border-radius: 12px; background: linear-gradient(135deg, #7c3aed, #a855f7); display: flex; align-items: center; justify-content: center; font-size: 16px; }
.stats-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 20px; }
.stat-card { background: var(--bg-card); border: 1px solid var(--border-card); border-radius: var(--radius); padding: 18px 16px; position: relative; overflow: hidden; backdrop-filter: blur(20px); transition: all 0.3s; }
.stat-card::before { content: ''; position: absolute; top: 0; right: 0; left: 0; height: 2px; }
.stat-card:nth-child(1)::before { background: linear-gradient(135deg, #7c3aed, #a855f7); }
.stat-card:nth-child(2)::before { background: linear-gradient(135deg, #10b981, #34d399); }
.stat-card:nth-child(3)::before { background: linear-gradient(135deg, #3b82f6, #60a5fa); }
.stat-card:nth-child(4)::before { background: linear-gradient(135deg, #f59e0b, #fbbf24); }
...
.chart-container { display: flex; align-items: flex-end; justify-content: space-between; height: 160px; gap: 6px; direction: ltr; }
.chart-bar-group { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 6px; height: 100%; justify-content: flex-end; }
.chart-bar { width: 100%; max-width: 32px; border-radius: 6px 6px 4px 4px; background: linear-gradient(180deg, #a855f7, #7c3aed); box-shadow: 0 0 12px rgba(124,58,237,0.3); transform-origin: bottom; animation: grow 0.8s cubic-bezier(0.34,1.56,0.64,1) backwards; }
...
.activity-time { font-size: 11px; color: var(--text-tertiary); font-weight: 500; }
...
.activity-item:nth-child(1){animation-delay:.3s} .activity-item:nth-child(2){animation-delay:.35s} .activity-item:nth-child(3){animation-delay:.4s}
</style>
</head>
<body>
<div class="container">
    ...
    <div class="stats-grid">
        <div class="stat-card"><div class="stat-header"><span class="stat-label">المستخدمون</span><div class="stat-icon">👥</div></div><div class="stat-value">12,847</div><span class="stat-change">↑ 12.5%</span></div>
        <div class="stat-card"><div class="stat-header"><span class="stat-label">الإيرادات</span><div class="stat-icon">💰</div></div><div class="stat-value">$128.5K</div><span class="stat-change">↑ 8.3%</span></div>
        <div class="stat-card"><div class="stat-header"><span class="stat-label">الطلبات</span><div class="stat-icon">📦</div></div><div class="stat-value">3,421</div><span class="stat-change">↑ 15.2%</span></div>
        <div class="stat-card"><div class="stat-header"><span class="stat-label">النمو</span><div class="stat-icon">📈</div></div><div class="stat-value">+24.6%</div><span class="stat-change">↑ مستقر</span></div>
    </div>
    <div class="section-card">
        <div class="section-header"><span class="section-title">نظرة أسبوعية</span><span class="section-badge">📊 مباشر</span></div>
    ...
    <div class="section-card" style="animation-delay:.35s">
        <div class="section-header"><span class="section-title">النشاط الأخير</span><span class="section-badge">🔔 اليوم</span></div>
        {% for a in activities %}
    ...
    <div class="server-info"><div class="server-info-badge"><span class="dot"></span>Flask {{ flask_version }} · Python {{ python_version }} · يعمل</div></div>
</div>
</body>
</html>
"""
...
def index():
    ...
    return render_template_string(DASHBOARD_HTML, weekly_data=weekly, activities=generate_activities(),
        current_date=datetime.now().strftime('%Y/%m/%d'), flask_version=flask_ver, python_version=f'{sys.version_info.major}.{sys.version_info.minor}')

@app.route('/api/stats')
def api_stats():
    return jsonify({'status':'ok','data':{'users':12847,'orders':3421,'revenue':128500,'growth':24.6}})
