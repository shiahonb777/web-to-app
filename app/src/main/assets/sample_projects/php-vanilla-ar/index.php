<?php
/**
 * عرض قدرات PHP — لوحة معلومات النظام
 * 
 * تطبيق PHP بدون إطار عمل، يعرض قدرات PHP على Android
 * معلومات النظام | الإضافات | النماذج | الجلسات | الأداء
 */
...
$info = [
    ...
    'SAPI' => php_sapi_name(),
    'نظام التشغيل' => PHP_OS . ' (' . php_uname('m') . ')',
    'برنامج الخادم' => $_SERVER['SERVER_SOFTWARE'] ?? 'N/A',
    'إصدار Zend' => zend_version(),
    'وقت التنفيذ الأقصى' => ini_get('max_execution_time') . 's',
    'حد الذاكرة' => ini_get('memory_limit'),
    'الذاكرة المستخدمة' => round(memory_get_usage(true) / 1024 / 1024, 2) . ' MB',
    'ذروة الذاكرة' => round(memory_get_peak_usage(true) / 1024 / 1024, 2) . ' MB',
    'وقت الخادم' => date('Y-m-d H:i:s'),
    'المنطقة الزمنية' => date_default_timezone_get(),
    'معرف العملية' => getmypid(),
];
...
.info-val{font-weight:600;text-align:left;word-break:break-all;color:var(--primary)}
...
.bench-val{width:70px;text-align:left;font-weight:700;font-size:.85em;color:#f59e0b}
...
<div class="hero">
    <h1>🐘 عرض قدرات PHP</h1>
    <p>استكشف PHP <?=phpversion()?> على جهاز Android</p>
    <div class="stats">
        <div class="stat"><div class="stat-num"><?=phpversion()?></div><div class="stat-label">إصدار PHP</div></div>
        <div class="stat"><div class="stat-num"><?=$extCount?></div><div class="stat-label">الإضافات</div></div>
        <div class="stat"><div class="stat-num"><?=PHP_INT_SIZE * 8?>bit</div><div class="stat-label">البنية</div></div>
        <div class="stat"><div class="stat-num"><?=PHP_OS?></div><div class="stat-label">النظام</div></div>
    </div>
</div>
<div class="container">
    <div class="section">
    ...
    <div class="section">
        ...
        $benchmarks = ["عمليات عدد صحيح" => $intTime, "عمليات نصية" => $strTime, "ترتيب مصفوفة" => $sortTime];
    ...
    <div class="section">
        ...
        <?php foreach ($features as $name => $ok): ?>
            <div class="feat"><span><?=$name?></span><span class="<?=$ok?'feat-yes':'feat-no'?>"><?=$ok?'✓ متاح':'✗ غير متاح'?></span></div>
        <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="section">
        <h2>🔐 إدارة الجلسات</h2>
        <form method="POST">
            <input name="session_key" placeholder="مفتاح" required>
            <input name="session_val" placeholder="قيمة" required>
        ...
        <?php if ($sessionData): ?>
            ...
            <?php endforeach; ?>
            <form method="POST" style="margin-top:8px"><input type="hidden" name="clear_session" value="1"><button class="btn btn-danger" type="submit">مسح الجلسة</button></form>
        <?php else: ?>
            <p style="color:var(--text2);font-size:.9em;padding:8px 0">لا توجد بيانات جلسة. جرب إضافة واحدة!</p>
        <?php endif; ?>
    </div>

    <div class="section">
        <h2>🧩 الإضافات (<?=$extCount?>)</h2>
        <div>
...
<footer>عرض قدرات PHP · PHP <?=phpversion()?> · يعمل على Android</footer>
</body>
</html>
