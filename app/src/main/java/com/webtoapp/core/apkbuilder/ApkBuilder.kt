...
class ApkBuilder(private val context: Context) {
    ...
    suspend fun buildApk(
        ...
        try {
            ...
            // 画廊配置
            logger.section("画廊配置")
            logger.logKeyValue("galleryConfig", webApp.galleryConfig)
            logger.logKeyValue("galleryConfig.items.size", webApp.galleryConfig?.items?.size ?: 0)
            webApp.galleryConfig?.items?.forEachIndexed { index, item ->
                logger.log("  item[$index]: type=${item.type}, path=${item.path}, title=${item.title}")
            }
            ...
            Log.d("ApkBuilder", "  galleryConfig.items.size=${webApp.galleryConfig?.items?.size ?: 0}")
            ...
            // 获取画廊媒体文件路径列表
            val galleryMediaPaths = webApp.galleryConfig?.items?.map { it.path } ?: emptyList()
            logger.logKeyValue("galleryMediaPaths.size", galleryMediaPaths.size)
            galleryMediaPaths.forEachIndexed { index, path ->
                val file = File(path)
                logger.log("  gallery[$index]: path=$path, exists=${file.exists()}")
            }
            ...
            modifyApk(
                ...
                galleryMediaPaths, webApp.galleryConfig
            ) { progress ->
    ...
    private fun modifyApk(
        ...
        galleryMediaPaths: List<String> = emptyList(),
        galleryConfig: com.webtoapp.data.model.GalleryConfig? = null,
        onProgress: (Int) -> Unit
    ) {
        logger.log("modifyApk 开始")
        logger.logKeyValue("galleryMediaPaths.size", galleryMediaPaths.size)
        logger.logKeyValue("galleryConfig", galleryConfig != null)
        ...
        ZipFile(sourceApk).use { zipIn ->
            ZipOutputStream(FileOutputStream(outputApk)).use { zipOut ->
                ...
                if (config.appType != "WEB" && mediaContentPath != null && galleryMediaPaths.isEmpty()) {
                ...
                // 嵌入画廊媒体文件（多媒体模式）
                if (galleryMediaPaths.isNotEmpty() && galleryConfig != null) {
                    logger.section("嵌入画廊媒体")
                    val embeddedCount = addGalleryMediaToAssets(zipOut, galleryMediaPaths, galleryConfig)
                    logger.logKeyValue("画廊媒体嵌入数量", embeddedCount)
                    if (embeddedCount == 0) {
                        logger.warn("画廊模式但没有成功嵌入任何媒体文件！")
                    }
                }
    ...
    /**
     * 将画廊媒体文件添加到 assets/gallery 目录
     * 使用 STORED（未压缩）方式，以支持 AssetManager.openFd()
     * 注意：只嵌入 IMAGE 和 VIDEO 类型，WEB 类型不需要嵌入（运行时加载URL）
     * @return 成功嵌入的文件数量
     */
    private fun addGalleryMediaToAssets(
        zipOut: ZipOutputStream,
        mediaPaths: List<String>,
        galleryConfig: com.webtoapp.data.model.GalleryConfig
    ): Int {
        logger.log("准备处理 ${mediaPaths.size} 个画廊项")
        
        var successCount = 0
        var skippedCount = 0
        
        mediaPaths.forEachIndexed { index, mediaPath ->
            try {
                // 获取对应的画廊项配置
                val galleryItem = galleryConfig.items.getOrNull(index)
                val itemType = galleryItem?.type ?: com.webtoapp.data.model.GalleryItemType.IMAGE
                
                logger.log("处理画廊项[$index]: type=${itemType.name}, path=$mediaPath")
                
                // WEB 类型不需要嵌入文件，运行时直接加载 URL
                if (itemType == com.webtoapp.data.model.GalleryItemType.WEB) {
                    logger.log("  跳过 WEB 类型（运行时加载URL）")
                    skippedCount++
                    return@forEachIndexed
                }
                
                val mediaFile = File(mediaPath)
                logger.logKeyValue("  exists", mediaFile.exists())
                
                if (!mediaFile.exists()) {
                    logger.error("画廊媒体文件不存在: $mediaPath")
                    return@forEachIndexed
                }
                
                if (!mediaFile.canRead()) {
                    logger.error("画廊媒体文件无法读取: $mediaPath")
                    return@forEachIndexed
                }
                
                val mediaBytes = mediaFile.readBytes()
                if (mediaBytes.isEmpty()) {
                    logger.warn("画廊媒体文件内容为空: $mediaPath")
                    return@forEachIndexed
                }
                
                // 根据类型确定扩展名和路径
                val assetPath = when (itemType) {
                    com.webtoapp.data.model.GalleryItemType.VIDEO -> "assets/gallery/media_$index.mp4"
                    com.webtoapp.data.model.GalleryItemType.IMAGE -> "assets/gallery/media_$index.png"
                    com.webtoapp.data.model.GalleryItemType.HTML -> "assets/gallery/html_$index/index.html"
                    else -> return@forEachIndexed  // WEB 类型已在上面处理
                }
                
                writeEntryStoredSimple(zipOut, assetPath, mediaBytes)
                logger.log("画廊媒体已嵌入: $assetPath (${mediaBytes.size / 1024} KB)")
                successCount++
            } catch (e: Exception) {
                logger.error("嵌入画廊媒体失败: $mediaPath", e)
            }
        }
        
        logger.log("画廊处理完成: 嵌入 $successCount 个, 跳过 $skippedCount 个 WEB 类型")
        return successCount
    }
    ...
    private fun writeConfigEntry(zipOut: ZipOutputStream, config: ApkConfig) {
        ...
        Log.d("ApkBuilder", "写入配置文件: galleryEnabled=${config.galleryEnabled}, galleryItems.size=${config.galleryItems.size}")
        config.galleryItems.forEachIndexed { index, item ->
            Log.d("ApkBuilder", "  galleryItem[$index]: type=${item.type}, assetPath=${item.assetPath}")
        }
...
fun WebApp.toApkConfig(packageName: String): ApkConfig {
    ...
    return ApkConfig(
        ...
        hideToolbar = webViewConfig.hideToolbar || 
            ...
            (galleryConfig?.items?.isNotEmpty() == true),
        ...
        // 画廊配置
        galleryEnabled = galleryConfig?.items?.isNotEmpty() == true,
        galleryItems = galleryConfig?.items?.mapIndexed { index, item ->
            // 根据类型确定 assetPath
            // WEB 和 HTML 类型不嵌入文件，直接使用原始路径/URL
            // IMAGE 和 VIDEO 类型嵌入到 assets 中
            val (assetPath, needsEmbed) = when (item.type) {
                com.webtoapp.data.model.GalleryItemType.VIDEO -> "gallery/media_$index.mp4" to true
                com.webtoapp.data.model.GalleryItemType.IMAGE -> "gallery/media_$index.png" to true
                com.webtoapp.data.model.GalleryItemType.WEB -> item.path to false  // 网页URL，不嵌入
                com.webtoapp.data.model.GalleryItemType.HTML -> "gallery/html_$index/index.html" to true  // HTML需要嵌入
            }
            GalleryShellItem(
                id = item.id,
                title = item.title,
                ...
                enableAudio = item.mediaConfig?.enableAudio ?: true,
                loop = item.mediaConfig?.loop ?: false,
                autoPlay = item.mediaConfig?.autoPlay ?: true,
                fillScreen = item.mediaConfig?.fillScreen ?: true
            )
        } ?: emptyList(),
        galleryAutoPlay = galleryConfig?.autoPlay ?: false,
        galleryAutoPlayInterval = galleryConfig?.autoPlayInterval ?: 5,
        galleryShowIndicator = galleryConfig?.showIndicator ?: true,
        galleryLoop = galleryConfig?.loop ?: true,
        galleryTransition = galleryConfig?.transitionType?.name ?: "SLIDE",
