...
class ApkTemplate(private val context: Context) {
    ...
    fun createConfigJson(config: ApkConfig): String {
        return """
        {
            ...
            "galleryItems": [${config.galleryItems.joinToString(",") { item ->
                """{"id":"${escapeJson(item.id)}","title":"${escapeJson(item.title)}","type":"${item.type}","assetPath":"${escapeJson(item.assetPath)}","sortOrder":${item.sortOrder},"enableAudio":${item.enableAudio},"loop":${item.loop},"autoPlay":${item.autoPlay},"fillScreen":${item.fillScreen}}"""
            }}],
            "galleryAutoPlay": ${config.galleryAutoPlay},
            "galleryAutoPlayInterval": ${config.galleryAutoPlayInterval},
            "galleryShowIndicator": ${config.galleryShowIndicator},
            "galleryLoop": ${config.galleryLoop},
            "galleryTransition": "${config.galleryTransition}",
...
data class ApkConfig(
    ...
    // 画廊配置
    val galleryEnabled: Boolean = false,       // 是否为画廊模式
    val galleryItems: List<GalleryShellItem> = emptyList(), // 画廊项列表
    val galleryAutoPlay: Boolean = false,      // 是否自动播放
    val galleryAutoPlayInterval: Int = 5,      // 自动播放间隔（秒）
    val galleryShowIndicator: Boolean = true,  // 是否显示指示器
    val galleryLoop: Boolean = true,           // 是否循环
    val galleryTransition: String = "SLIDE",   // 切换动画类型
