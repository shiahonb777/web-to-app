...
class JarSigner(private val context: Context) {
    ...
    enum class SignerType {
        ...
        PKCS12_AUTO,       // 自动生成的 PKCS12
        PKCS12_CUSTOM      // 用户导入的自定义 PKCS12
    }
    ...
    private fun tryGenerateAndroidKeyStoreKey(): Boolean {
        ...
        return try {
            ...
            val notBefore = Date()
            val notAfter = Date(System.currentTimeMillis() + VALIDITY_YEARS * 365 * 24 * 60 * 60 * 1000)
    ...
    private fun generateSelfSignedCertificate(keyPair: KeyPair): X509Certificate {
        ...
        val notAfter = Date(now + VALIDITY_YEARS * 365 * 24 * 60 * 60 * 1000)
    ...
    private fun buildValidity(notBefore: Date, notAfter: Date): ByteArray {
        val out = ByteArrayOutputStream()
        out.write(encodeUTCTime(notBefore))
        out.write(encodeUTCTime(notAfter))
        return wrapWithTag(0x30, out.toByteArray())
    }
    ...
    private fun encodeUTCTime(date: Date): ByteArray {
        ...
        return wrapWithTag(0x17, timeStr.toByteArray(Charsets.US_ASCII))
    }
    ...
    private fun trySignWithRetry(inputApk: File, outputApk: File, maxRetries: Int): Boolean {
        ...
        for (attempt in 1..maxRetries) {
            try {
                ...
                if (success) {
                    Log.d(TAG, "签名成功 (尝试 $attempt)")
                    return true
                }

                // 签名返回 false，但没有异常，可能是验证失败
                Log.w(TAG, "签名尝试 $attempt 失败，验证未通过")

            } catch (e: Exception) {
                lastException = e
                Log.e(TAG, "签名尝试 $attempt 发生异常: ${e.message}", e)
        ...
        Log.e(TAG, "签名最终失败，已重试 $maxRetries 次", lastException)
        return false
    }
    ...
    private fun performSign(inputApk: File, outputApk: File): Boolean {
        ...
        val builder = ApkSigner.Builder(listOf(signerConfig))
            ...
            .setV3SigningEnabled(true)    // 启用 v3 签名
            .setMinSdkVersion(24)         // 与项目 minSdk 一致

        Log.d(TAG, "调用 ApkSigner.sign()...")
        builder.build().sign()
        Log.d(TAG, "ApkSigner.sign() 完成")

        // 验证签名结果
        if (!outputApk.exists() || outputApk.length() == 0L) {
            Log.e(TAG, "签名后输出文件不存在或为空")
            return false
        }
