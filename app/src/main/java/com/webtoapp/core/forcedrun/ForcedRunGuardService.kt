...
class ForcedRunGuardService : Service() {
    ...
    private fun getCurrentForegroundPackage(): String? {
        ...
        // 方法2：使用 ActivityManager（需要特殊权限或旧版本）
        try {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                CHANNEL_ID,
                Strings.forcedRunChannelName,
                NotificationManager.IMPORTANCE_LOW
            ).apply {
                description = Strings.forcedRunChannelDescription
                setShowBadge(false)
            }
    ...
    private fun createNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle(Strings.forcedRunActive)
            .setContentText(Strings.forcedRunReturnToApp)
