    ...
    suspend fun extractSampleProject(
        ...
        return try {
            ...
            if (!forceRefresh && 
                ...
                Log.d(TAG, "示例项目已存在且版本匹配: ${outputDir.absolutePath}")
                return Result.success(outputDir.absolutePath)
            }
            
            Log.i(TAG, "重新解压示例项目 (版本: $cachedVersion -> $currentVersion)")
            ...
            Log.i(TAG, "示例项目已解压: ${outputDir.absolutePath}")
            Result.success(outputDir.absolutePath)
            
        } catch (e: Exception) {
            Log.e(TAG, "解压示例项目失败", e)
            Result.failure(e)
        }
    }
