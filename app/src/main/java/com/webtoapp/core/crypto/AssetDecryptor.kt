...
class AssetDecryptor(private val context: Context) {
    ...
    private fun getKey(): SecretKey {
        ...
        synchronized(this) {
            ...
            val key = if (metadata != null && metadata.signatureHash.isNotBlank()) {
    ...
    private fun String.hexToByteArray(): ByteArray {
        check(length % 2 == 0) { "十六进制字符串长度必须是偶数" }
    ...
    fun loadAsset(assetPath: String): ByteArray {
        ...
        return try {
            ...
            Log.d(TAG, "加载加密资源: $encryptedPath")
            ...
            try {
                ...
                Log.e(TAG, "资源不存在: $assetPath", e2)
                throw CryptoException("资源不存在: $assetPath", e2)
            }
        }
    }
