    ...
    fun load(): Boolean {
        ...
        synchronized(this) {
            ...
            isAvailable = try {
                ...
                Log.i(TAG, "Native crypto library loaded")
                true
            } catch (e: UnsatisfiedLinkError) {
                Log.w(TAG, "Failed to load native crypto library: ${e.message}")
                false
            } catch (e: Exception) {
                Log.e(TAG, "Error loading native crypto library", e)
                false
            }
...
class HybridDecryptor(private val context: Context) {
    ...
    private val nativeAvailable = NativeCrypto.isNativeAvailable()
    ...
    fun decrypt(encrypted: ByteArray): ByteArray {
        ...
        if (nativeAvailable) {
            ...
            Log.w("HybridDecryptor", "Native decryption failed, falling back to Java")
        }
