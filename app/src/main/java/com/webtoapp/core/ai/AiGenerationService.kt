...
class AiGenerationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        startForeground(NOTIFICATION_ID, createNotification("AI 生成服务运行中"))
        return START_STICKY
    }
    ...
    fun startGeneration(
        ...
        updateNotification("正在生成 HTML 代码...")
        ...
        currentJob = serviceScope.launch {
            try {
                htmlAgent?.developWithStream(
                    ...
                    when (event) {
                        is HtmlAgentEvent.CodeDelta -> {
                            updateNotification("正在生成代码... ${event.accumulated.length} 字符")
                        }
                        is HtmlAgentEvent.FileCreated -> {
                            val versionInfo = if (event.isNewVersion) "v${event.fileInfo.version}" else "新建"
                            updateNotification("文件已创建: ${event.fileInfo.name} ($versionInfo)")
                        ...
                        is HtmlAgentEvent.HtmlComplete -> {
                            ...
                            updateNotification("代码生成完成")
                        }
                        is HtmlAgentEvent.Error -> {
                            updateNotification("生成失败: ${event.message}")
                ...
                _eventFlow.emit(HtmlAgentEvent.Error("生成已取消"))
                releaseWakeLock()
            } catch (e: Exception) {
                Log.e(TAG, "Generation error", e)
                _eventFlow.emit(HtmlAgentEvent.Error("生成失败: ${e.message}"))
                releaseWakeLock()
            } finally {
                _isGenerating.value = false
            }
