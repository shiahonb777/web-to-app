...
object CodeBlockParser {
    ...
    // 清理残留的思考标记
    private val thinkingCleanupRegex = Regex(
        """\[?THINKING\]?:?\s*|""" +
        """【思考】|【/思考】|""" +
        """</?thinking>|</?think>""",
    ...
    // 需要跳过的代码块类型（已单独处理）
    private val skipLanguages = setOf("thinking", "image-gen")
    ...
    fun parseResponse(response: String): ParsedAiResponse {
        ...
        thinkingRegex.find(response)?.let { match ->
            // 找到第一个非空的捕获组
            thinking = match.groupValues.drop(1).firstOrNull { it.isNotEmpty() }?.trim()
            textContent = textContent.replace(match.value, "")
        }
        
        // 清理残留的思考标记
        textContent = thinkingCleanupRegex.replace(textContent, "")
        ...
        codeBlockRegex.findAll(response).forEach { match ->
            ...
            // 跳过已单独处理的代码块类型
            if (language in skipLanguages) return@forEach
            
            // 从内容中提取文件名
            val filename = extractFilenameFromContent(rawContent)
            
            // 清理内容中的文件名注释
            val content = filenameRegex.replace(rawContent, "").trim()
    ...
    private fun extractFilenameFromContent(content: String): String? {
        return filenameRegex.find(content)?.let { match ->
            match.groupValues.drop(1).firstOrNull { it.isNotEmpty() }?.trim()
        }
    }
