...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun editApp(webApp: WebApp) {
        ...
        _editState.value = EditState(
            name = webApp.name,
            url = webApp.url,
            // 使用 Uri.fromFile 确保文件路径正确显示
            iconUri = webApp.iconPath?.let { path ->
                if (path.startsWith("/")) Uri.fromFile(java.io.File(path))
                else Uri.parse(path)
            },
            ...
            // 使用 Uri.fromFile 确保文件路径正确显示
            splashMediaUri = webApp.splashConfig?.mediaPath?.let { path ->
                if (path.startsWith("/")) Uri.fromFile(java.io.File(path))
                else Uri.parse(path)
            },
    ...
    fun handleIconSelected(uri: Uri) {
        viewModelScope.launch {
            ...
            if (savedPath != null) {
                ...
                // 更新状态，保存本地路径
                // 使用 Uri.fromFile 确保文件路径正确转换为 file:// URI
                _editState.value = _editState.value.copy(
                    iconUri = Uri.fromFile(java.io.File(savedPath)),
     ...
     * 标准化URL
     * - 自动添加 https:// 前缀
     * - 修复以点开头的无效域名（如 .m.bilibili.com -> m.bilibili.com）
     * - 清理多余的空格和特殊字符
     */
    private fun normalizeUrl(url: String): String {
        ...
        // 修复无效域名：移除协议后域名开头的点（如 https://.m.xxx -> https://m.xxx）
        trimmed = trimmed.replace(Regex("^(https?://)\\.(\\w)"), "$1$2")
