    ...
    NavHost(
        ...
        composable(Routes.CREATE_MEDIA_APP) {
            CreateMediaAppScreen(
                onBack = { navController.popBackStack() },
                onCreated = { name, appType, mediaUri, mediaConfig, iconUri, themeType, galleryConfig ->
                    viewModel.saveMediaApp(
                        name, appType, mediaUri, mediaConfig, iconUri, themeType, galleryConfig
                    )
        ...
        composable(Routes.CREATE_HTML_APP) {
            CreateHtmlAppScreen(
                onBack = { navController.popBackStack() },
                onCreated = { name, htmlConfig, iconUri, themeType, galleryConfig ->
                    viewModel.saveHtmlApp(
                        name, htmlConfig, iconUri, themeType, galleryConfig
                    )
                    navController.popBackStack()
                }
            )
