...
class Converters {
    ...
    // TranslateConfig 转换
    @TypeConverter
    fun fromTranslateConfig(value: TranslateConfig?): String = gson.toJson(value)

    @TypeConverter
    fun toTranslateConfig(value: String): TranslateConfig? = try {
        gson.fromJson(value, TranslateConfig::class.java)
    } catch (e: Exception) {
        null
    }
    ...
    fun toActivationCodeList(value: String): List<ActivationCode> {
        val type = object : TypeToken<List<ActivationCode>>() {}.type
        return try {
            gson.fromJson(value, type) ?: emptyList()
        } catch (e: Exception) {
            emptyList()
    ...
    // GalleryConfig 转换
    @TypeConverter
    fun fromGalleryConfig(value: GalleryConfig?): String = gson.toJson(value)

    @TypeConverter
    fun toGalleryConfig(value: String): GalleryConfig? = try {
        gson.fromJson(value, GalleryConfig::class.java)
    } catch (e: Exception) {
        null
    }
}
