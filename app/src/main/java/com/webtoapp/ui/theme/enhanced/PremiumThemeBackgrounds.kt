    ...
    Canvas(
        modifier = modifier
            ...
            .pointerInput(Unit) {
                detectDragGestures(
                    onDragStart = { offset ->
                        dragPosition = Vec2(offset.x, offset.y)
                    },
                    onDrag = { change, _ ->
                        val newPos = Vec2(change.position.x, change.position.y)
                        dragPosition?.let { oldPos ->
                            val velocity = newPos - oldPos
...
class ForestPhysicsWorld(val bounds: PhysicsBounds) {
    ...
    fun initialize() {
        ...
        // 初始化生态系统
        ecosystem.initialize(plantCount = 15, preyCount = 8, predatorCount = 1)
    }
    ...
    fun update(dt: Float, sensorData: SensorData, touchManager: TouchInteractionManager) {
        ...
        // 补充树叶
        if (leaves.size < 25 && Random.nextFloat() < 0.03f) {
            leaves.add(createLeaf())
        }
    }
    ...
    private fun updateFireflies(dt: Float, sensorData: SensorData, touchManager: TouchInteractionManager) {
        ...
        // Boids行为参数
        val separationDist = 25f
        val alignmentDist = 60f
        val cohesionDist = 100f
...
private fun DrawScope.drawFireflies(fireflies: List<Firefly>, time: Float) {
    for (firefly in fireflies) {
        ...
        drawCircle(
            brush = Brush.radialGradient(
                colors = listOf(
                    firefly.color.copy(alpha = alpha * 0.15f),
                    firefly.color.copy(alpha = alpha * 0.03f),
                    Color.Transparent
                )
            ),
            radius = currentSize * 10f,
            center = Offset(firefly.position.x, firefly.position.y)
        )
        ...
        drawCircle(
            brush = Brush.radialGradient(
                colors = listOf(
                    ...
                    firefly.color.copy(alpha = alpha * 0.5f),
                    Color.Transparent
                )
            ),
