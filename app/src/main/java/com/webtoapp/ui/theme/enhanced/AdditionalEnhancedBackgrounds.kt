...
fun MidnightEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            stars = List(150) { createMidnightStar(canvasSize, it) }
            glowParticles = List(30) { createGlowParticle(canvasSize) }
...
fun EmeraldEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            crystals = List(25) { createEmeraldCrystal(canvasSize) }
            sparkles = List(60) { createEmeraldSparkle(canvasSize) }
            lightRays = List(8) { createLightRay(canvasSize, it) }
...
fun RoseGoldEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            goldFlakes = List(80) { createGoldFlake(canvasSize) }
            softGlows = List(15) { createSoftGlow(canvasSize) }
            starPoints = List(40) { createStarPoint(canvasSize) }
            ...
            while (true) {
                ...
                // 补充金箔
                if (goldFlakes.size < 70 && Random.nextFloat() < 0.1f) {
