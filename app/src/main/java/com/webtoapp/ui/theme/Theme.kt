 ...
 * 用于导出的 APK 应用
 * 
 * @param themeTypeName 主题类型名称（对应 AppThemeType 枚举的 name）
...
fun ShellTheme(
    ...
    // 选择配色方案
    val colorScheme = if (darkTheme) currentTheme.darkColors else currentTheme.lightColors
    
    CompositionLocalProvider(
        LocalAppTheme provides currentTheme,
        LocalAnimationSettings provides AnimationSettings(
            enabled = true,
            particlesEnabled = false,  // Shell 模式关闭粒子效果以优化性能
            hapticsEnabled = true,
