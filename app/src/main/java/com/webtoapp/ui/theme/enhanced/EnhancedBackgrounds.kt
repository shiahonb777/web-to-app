...
fun AuroraEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            auroraParticles = List(80) { createAuroraParticle(canvasSize) }
            while (true) {
                delay(16)
                frame++
        ...
        modifier = modifier
            ...
            .pointerInput(Unit) {
                detectTapGestures { offset ->
                    ...
                    // 点击生成极光爆发
                    if (canvasSize.width > 0) {
                        auroraParticles = auroraParticles + List(20) {
                            AuroraParticle(
...
private fun DrawScope.drawStarfield(
    ...
    val layers = listOf(
        Triple(150, 0.3f, 1.5f),  // 远景星星
        Triple(80, 0.6f, 2.5f),   // 中景星星
        Triple(30, 1f, 4f)        // 近景星星
    )
    ...
    layers.forEachIndexed { layerIndex, (count, parallax, maxSize) ->
        ...
        repeat(count) {
            ...
            val twinkle = (sin(frame * 0.05f * (1 + random.nextFloat()) + random.nextFloat() * PI.toFloat() * 2) + 1) / 2
            val alpha = (0.3f + twinkle * 0.7f) * parallax
...
fun SakuraEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            petals = List(50) { createSakuraPetal(canvasSize) }
            fallingPetals = List(15) { createTreePetal(canvasSize) }
...
fun OceanEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            ...
            bubbles = List(35) { createBubble(canvasSize) }
            jellyfish = List(4) { createJellyfish(canvasSize) }
            caustics = List(20) { createCaustic(canvasSize) }
        ...
        modifier = modifier
            ...
            .pointerInput(Unit) {
                detectDragGestures { change, dragAmount ->
                    ...
                    wavePoints = wavePoints.map { point ->
                        val dist = abs(point.x - x)
                        if (dist < 60) {
                            point.copy(velocity = point.velocity + dragAmount.y * 0.15f)
                        } else point
                    }
                }
            }
...
fun GalaxyEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            stars = List(250) { createStar3D(canvasSize) }
            nebulaClouds = List(5) { createNebulaCloud(canvasSize) }
...
fun CyberpunkEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            dataStreams = List(15) { createDataStream(canvasSize) }
            holoParticles = List(40) { createHoloParticle(canvasSize) }
            ...
            .pointerInput(Unit) {
                detectTapGestures { offset ->
                    ...
                    if (canvasSize.width > 0) {
                        ...
                        // 添加全息粒子爆发
                        holoParticles = holoParticles + List(20) {
                            val angle = Random.nextFloat() * 2 * PI.toFloat()
...
fun VolcanoEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            fireParticles = List(60) { createFireParticle(canvasSize) }
            lavaBlobs = List(12) { createLavaBlob(canvasSize) }
            ...
            while (true) {
                ...
                // 喷发效果
                if (erupting && frame % 3 == 0L) {
                    sparks = sparks + List(5) {
                        Spark(
            ...
            .pointerInput(Unit) {
                detectTapGestures { offset ->
                    ...
                    if (canvasSize.width > 0) {
                        ...
                        fireParticles = fireParticles + List(15) {
                            FireParticle(
...
fun FrostEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            snowflakes = List(100) { createSnowflake(canvasSize) }
            iceCrystals = List(12) { createIceCrystal(canvasSize) }
            frostPatches = List(8) { createFrostPatch(canvasSize) }
