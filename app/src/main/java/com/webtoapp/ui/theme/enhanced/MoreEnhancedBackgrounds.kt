...
fun ForestEnhancedBackground(
    ...
    LaunchedEffect(canvasSize) {
        if (canvasSize.width > 0) {
            // 初始化粒子
            fireflies = List(60) { createForestFirefly(canvasSize) }
            leaves = List(25) { createForestLeaf(canvasSize) }
            lightSpeckles = List(40) { createLightSpeckle(canvasSize) }
            ...
            while (true) {
                ...
                // 补充树叶
                if (leaves.size < 20 && Random.nextFloat() < 0.05f) {
