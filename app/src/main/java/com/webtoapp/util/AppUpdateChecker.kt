    ...
    suspend fun checkUpdate(
        ...
        repeat(MAX_RETRIES) { attempt ->
            try {
                ...
                lastException = e
                android.util.Log.w("AppUpdateChecker", "Update check failed (attempt ${attempt + 1}/$MAX_RETRIES)", e)
            }
        ...
        Result.failure(lastException ?: Exception(Strings.updateCheckFailed))
    }
        ...
        return try {
            ...
            if (!response.isSuccessful) {
                return Result.failure(
                    Exception(Strings.networkRequestFailedWithCodeFormat.format(response.code))
                )
            }
            ...
            if (latestVersion.isEmpty()) {
                return Result.failure(Exception(Strings.updateVersionNotFound))
            }
            ...
            android.util.Log.e("AppUpdateChecker", "Update check failed", e)
            Result.failure(e)
        }
    }
    ...
    fun downloadApk(
        ...
        return try {
            ...
            val request = DownloadManager.Request(Uri.parse(downloadUrl))
                ...
                .setDescription(context.getString(R.string.update_download_description, versionName))
