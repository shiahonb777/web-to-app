...
object AppUpdateChecker {
    ...
    /**
     * 获取当前应用版本信息
     * 使用 parseVersionCode 从 versionName 解析版本号，确保与远程版本比较逻辑一致
     */
    fun getCurrentVersionInfo(context: Context): Pair<String, Int> {
        return try {
            val packageInfo = context.packageManager.getPackageInfo(context.packageName, 0)
            // 清理 versionName 中的零宽空格等不可见字符
            val rawVersionName = packageInfo.versionName ?: "1.0.0"
            val versionName = rawVersionName.replace(Regex("[\\u200B-\\u200D\\uFEFF]"), "").trim()
            // 使用与远程版本相同的解析方法，确保比较一致
