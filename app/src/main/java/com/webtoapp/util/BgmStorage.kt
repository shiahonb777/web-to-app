...
object BgmStorage {
    ...
    fun scanAssetsBgm(context: Context): List<BgmItem> {
        ...
        try {
            ...
            for (mp3File in mp3Files) {
                ...
                // 查找同名 LRC 文件（先检查用户目录，再检查 assets）
    ...
    fun saveBgm(context: Context, uri: Uri, customName: String? = null): String? {
        return try {
            ...
            if (inputStream == null) {
                android.util.Log.e("BgmStorage", "Unable to open audio file: $uri")
                return null
            }
            ...
            inputStream.use { input ->
                FileOutputStream(destFile).use { output ->
                    ...
                    android.util.Log.d("BgmStorage", "Audio file saved: ${destFile.absolutePath}, size: $bytes bytes")
                }
            }

            // 验证文件是否成功保存
            if (!destFile.exists() || destFile.length() == 0L) {
                android.util.Log.e("BgmStorage", "Audio file save failed or empty: ${destFile.absolutePath}")
                return null
            }
