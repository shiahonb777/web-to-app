...
class DownloadNotificationManager(private val context: Context) {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                CHANNEL_ID,
                Strings.downloadNotificationsTitle,
                NotificationManager.IMPORTANCE_LOW
            ).apply {
                description = Strings.downloadNotificationsDescription
                setShowBadge(false)
            }
    ...
    private fun showProgressNotification() {
        ...
        val title = if (downloadingCount == 1) {
            Strings.downloadingFileFormat.format(activeDownloads.values.first().fileName)
        } else {
            Strings.downloadingFilesFormat.format(downloadingCount)
        }
        ...
        cursor?.use {
            if (it.moveToFirst()) {
                ...
                if (statusIndex >= 0) {
                    ...
                    val title = if (titleIndex >= 0) it.getString(titleIndex)
                    else info?.fileName ?: Strings.fileLabel
    ...
    private fun showDownloadCompleteNotification(
        ...
        val sharePendingIntent = if (shareIntent != null) {
            PendingIntent.getActivity(
                ...
                Intent.createChooser(shareIntent, Strings.shareFile),
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )
        } else null
        ...
        val builder = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentTitle(Strings.downloadComplete)
    ...
    private fun showDownloadFailedNotification(downloadId: Long, fileName: String) {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentTitle(Strings.downloadFailedTitle)
            .setContentText(fileName)
            .setAutoCancel(true)
            .setPriority(NotificationCompat.PRIORITY_DEFAULT)
            .build()
    ...
    private fun createOpenFileIntent(localUri: String?, mimeType: String): Intent? {
        ...
        return try {
            ...
            Log.e(TAG, "Failed to create open-file intent", e)
            null
        }
    }
    ...
    private fun createShareFileIntent(localUri: String?, mimeType: String): Intent? {
        ...
        return try {
            ...
            Log.e(TAG, "Failed to create share-file intent", e)
            null
        }
    }
    ...
    fun showIndeterminateProgress(fileName: String): Int {
        if (!hasNotificationPermission()) {
            Log.w(TAG, "No notification permission; skipping progress notification")
            return -1
        }
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(android.R.drawable.stat_sys_download)
            .setContentTitle(Strings.saving)
            .setContentText(fileName)
    ...
    fun showProgress(fileName: String, progress: Int, totalBytes: Long, downloadedBytes: Long): Int {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(android.R.drawable.stat_sys_download)
            .setContentTitle(Strings.downloading)
    ...
    fun showSaveComplete(
        ...
        if (!hasNotificationPermission()) {
            Log.w(TAG, "No notification permission; skipping completion notification")
            return
        }
        ...
        val builder = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(android.R.drawable.stat_sys_download_done)
            .setContentTitle(Strings.saveComplete)
            .setContentText(fileName)
            .setAutoCancel(true)
            .setPriority(NotificationCompat.PRIORITY_DEFAULT)
        ...
        notificationManager.notify(notificationId, builder.build())
        Log.d(TAG, "Save complete notification shown: $fileName")
    }
    ...
    fun showMediaSaveComplete(
        ...
        val sharePendingIntent = PendingIntent.getActivity(
            ...
            Intent.createChooser(
                shareIntent,
                Strings.shareTypeFormat.format(typeText)
            ),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(android.R.drawable.stat_sys_download_done)
            .setContentTitle(Strings.savedToGalleryFormat.format(typeText))
            .setContentText(fileName)
            .setContentIntent(openPendingIntent)
            .addAction(android.R.drawable.ic_menu_view, Strings.view, openPendingIntent)
            .addAction(android.R.drawable.ic_menu_share, Strings.share, sharePendingIntent)
            .setAutoCancel(true)
            .setPriority(NotificationCompat.PRIORITY_DEFAULT)
            .build()

        notificationManager.notify(notificationId, notification)
        Log.d(TAG, "$typeText save complete notification shown: $fileName")
    }
    ...
    fun showSaveFailed(
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(android.R.drawable.stat_notify_error)
            .setContentTitle(Strings.saveFailed)
        ...
        notificationManager.notify(notificationId, notification)
        Log.d(TAG, "Save failed notification shown: $fileName - $errorMessage")
    }
