...
object HtmlProjectProcessor {
    ...
    private fun fixResourcePaths(html: String): String {
        ...
        result = result.replace(Regex("""(href|src)=["'](/[^"']+)["']""")) { match ->
...
$trimmed
    ...
    private fun analyzeHtmlReferences(html: String, baseDir: File?): List<ResourceReference> {
        ...
        // CSS link 标签
        Regex("""<link[^>]*href=["']([^"']+)["'][^>]*>""", RegexOption.IGNORE_CASE).findAll(html).forEach { match ->
        ...
        // JS script 标签
        Regex("""<script[^>]*src=["']([^"']+)["'][^>]*>""", RegexOption.IGNORE_CASE).findAll(html).forEach { match ->
            val path = match.groupValues[1]
